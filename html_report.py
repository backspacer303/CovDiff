...
# Klasa zaduzena za formiranje svih zadatih formata prikaza.
# Za svaki format, stranice za pojedinacne datoteke se generisu paralelno.
class ReportDisplayGenerator:

    def __init__(self, reportDisplayList, projectCoverage1, projectCoverage2, numOfWorkers):

        # Lista formata prikaza koje je potrebno generisati.
        # Formati moraju implementirati metode definisane u klasi ReportDisplay.
        self.reportDisplayList = reportDisplayList

        # Lista svih naziva izvornih datoteka za koje postoje informacije o pokrivenosti koda.
        self.allSFList = list(set(projectCoverage1.reports.keys()).union(set(projectCoverage2.reports.keys())))
        # Broj radnika za paralelno formiranje pojedinacnih stranica koje odgovaraju izvornim datotekama. 
        self.numOfWorkers = numOfWorkers

        # Deljeni brojac medju radnicima. Koristi se kao sufkis za ime stranice
        # ukoliko vec postoji generisana stranica sa tim imenom.
        self.numOfSameSFNames = 1

        # Mapa koja preslikava naziv izvorne datoteke u putanju do stranice koja joj odgovara.
        self.SFToPage = {}

        # Sluazi da se prosledi referenca na trenutni format prikaza
        # funkciji koju izvrsavaju deca-procesi.
        self.curretReportDisplay = None
    ...
    def generateIndividualPagesforAllSF(self):
        ...
        # NAPOMENA: Pri generisanju stranica u zadatom formatu pomocu niti procesa problem pravi
        ...
        with multiprocessing.Pool(processes=self.numOfWorkers) as pool:
            ...
            for r in results.get():
                self.SFToPage[r[0]] = r[1]

            pool.close()
            pool.join()

    # Funkica koju izvrsava svaki radnik.
    def threadFn(self, sourceFile):

        pagePath, pageContent = self.curretReportDisplay.generateSFPage(sourceFile)
        ...
        if os.path.exists(pagePath):
            ...
            # Potrebno sinhronizovati njegovo koriscenje da ne bi doslo do pogresnih rezultata.
            with numOfSameSFNames_lock:
                pagePath_root, pagePath_ext = os.path.splitext(pagePath)
                pagePath = pagePath_root + "_" + str(self.numOfSameSFNames) + pagePath_ext
                self.numOfSameSFNames += 1

        with open(pagePath, "w") as f:
            f.write(pageContent)

        return (sourceFile, pagePath)

    # Funkcija formira datoteku koja predstavlja glavnu stranicu.
    def generateMainPage(self):
        pagePath, pageContent = self.curretReportDisplay.generateMainPage(self.SFToPage)
        with open(pagePath, "w") as f:
            f.write(pageContent)
    ...
    def generate(self):
        ...
        # Polazi se kroz listu zadatih formata i vrsi se njihovo kreiranje.
        for reportDisplay in self.reportDisplayList:

            reportDisplay.prepare()
...
# Klasa definise metode koje sve kalse za generisanje prikaza rezultata
# treba da implementiraju.
class ReportDisplay:
    # Priprema (pravljenje direktorijuma, kopiranje dodatnih datoteka itd.)
    def prepare(self):
        pass
    # Generisanje glavne stranice prikaza.
    def generateMainPage(self, SFToPage) -> (str, str):
        pass
    # Generisanje liste sa spiskom svih pogodjenih izvornih datoteka.
    def generateAllSFList(self, SFToPage):
        pass
    # Generisanje liste sa spiskom svih izvornih datoteka u kojima postoje razlike
    # u pokrivenosti koda sa dva testa.
    def generateSFDiffList(self, SFToPage):
        pass
    # Generisanje MiniReport izvestaja.
    def generateMiniReport(self, miniReport, testName):
        pass
    # Generisanje pojedinacnih stranica koje odgovaraju izvornim datotekama.
    def generateSFPage(self, sourceFile) -> (str, str):
        pass
    # Generisanje sumarnog prikaza pokrivenih linija.
    def generateCoveredLines(self, sourceFile, lines):
        pass
    # Generisanje sumarnog prikaza pokrivenih funkcija.
    def generateCoveredFunctions(self, sourceFile):
        pass
    # Generisanje prikaza razlika u pokrivenosti koda testovima.
    def generateCoverageDiff(self, sourceFile, lines):
        pass
    # Generisanje uporednog prikaza pokrivenosti koda testovima.
    def generateSideBySideCoverage(self, sourceFile, lines):
        pass


# Klasa generise finalni prikaz razlika u pokrivenosti koda u formatu html.
# Zaduzena je za formiranje sadrzaja pocetna stranice i stranica 
# za pojedinacne izvorne datoteke.
class HtmlReportDisplay(ReportDisplay):
    ...
    def generateMainPage(self, SFToPage):
        ...
        with a.html():
            ...
            with a.body():
                ...
                with a.div(klass="content", style="display: none;"):
                    ...
                    a(self.generateAllSFList(SFToPage))
                ...
                with a.div(klass="content", style="display: none;"):
                    ...
                    a(self.generateSFDiffList(SFToPage))
    ...
    def generateAllSFList(self, SFToPage):
        ...
        with a.table(klass="mainTable", id="AllSFTable"):
            ...
            for file in sourceFileUnion:
                ...
                pagePath = SFToPage[file]
                index = pagePath.find('Pages')
                if index == -1:
                    print("OGROMNA GRESKA")
                    exit()
                htmlPageLink = "./" + pagePath[index:]
    ...
    # Funkcija generise sadrzaj stranica za pojedninace izvorne datoteke.
    # Vraca sadzaj u vidu niske i putanju na kojoj je potrebno kreirati stranicu.
    # Stranica sadrzi:
    #    - Zbirni izvesta
        ...
        with a.html():
            ...
            with a.body():
                ...
                with a.div(klass="content", style="display: none;"):
                    ...
                    a(self.generateCoveredLines(sourceFile, lines))
                ...
                with a.div(klass="content", style="display: none;"):
                    ...
                    a(self.generateCoveredFunctions(sourceFile))
                ...
                with a.div(klass="content", style="display: none;"):
                    ...
                    a(self.generateCoverageDiff(sourceFile, lines))
                ...
                with a.div(klass="content", style="display: none;"):
                    ...
                    a(self.generateSideBySideCoverage(sourceFile, lines))
